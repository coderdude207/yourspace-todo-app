...
var connection = mysql.createConnection({
  ...
  user: "oc2nmyongr8lgutt",
  password: "jpa2zt4shao0ifxj",
  database: "fjdd8lijbh1rrrj6",
});
...
var transporter = nodemailer.createTransport({
  ...
  auth: {
    user: "apikey",
    pass:
      "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",
  },
});
...
Please do not share it with anyone.`,
  ...
  var check =
    ' select count(email) AS val from registration where email="' +
    email +
    '"  ';
    ...
    if (checkemail[0].val == 1) {
      ...
      connection.query(sql, function (err) {
        ...
        connection.query(otpsql, function (err) {
          if (err) throw err;
          var imagesql =
            "insert into userprofileimages values('" +
            noimage +
            "','" +
            email +
            "')";
...
app.post("/otp/verify", function (req, res) {
  ...
  connection.query(
    "SELECT * FROM verifyotp WHERE BINARY  otp = ? ",
    [otp],
    function (error, results, fields) {
      if (results.length > 0) {
        var verify = "true";
        var verifyed =
          " UPDATE verifyotp SET verify ='" +
          verify +
          "'  WHERE otp = '" +
          otp +
          "' ";
...
app.post("/login/verification", function (req, res) {
  ...
  if (email && password) {
    connection.query(
      "SELECT * FROM registration WHERE BINARY  email = ? AND  BINARY  password = ?",
      [email, password],
      function (error, results, fields) {
        if (results.length > 0) {
    ...
    connection.query(vv, function (err, verfy, fields) {
      ...
      if (verfy[0].verify !== "true") {
        ...
        connection.query(dash, function (err, users, fields) {
          if (err) throw err;
          var imgsql =
            'select * from userprofileimages where email="' + email + '"';
...
app.post("/profile/update/username", function (req, res) {
  if (req.session.loggedin) {
    let username = req.body.username;
    var sql =
      "UPDATE registration SET username='" +
      username +
      "'  WHERE email='" +
      req.session.email +
      "' ";
...
app.post("/profile/update/gender", function (req, res) {
  if (req.session.loggedin) {
    let gender = req.body.gender;
    var sql =
      "UPDATE registration SET gender='" +
      gender +
      "'  WHERE email='" +
      req.session.email +
      "' ";
...
app.post(
  ...
  upload.single("updateprofileimgage"),
  function (req, res, next) {
    if (req.session.loggedin) {
      var fileinfo = req.file.filename;
      var s =
        "select * from  userprofileimages where email='" +
        req.session.email +
        "' ";
      connection.query(s, function (error, results, fields) {
        if (results == "") {
          ...
          var fileinfo = req.file.filename;
          sa =
            "UPDATE userprofileimages  SET src='" +
            fileinfo +
            "' WHERE email='" +
            req.session.email +
            "';";
...
`,
    ...
    let checkIfExist =
      'select count(id) AS idcheck from task  where email="' +
      email +
      '" AND id="' +
      id +
      '" ';
